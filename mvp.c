...
int n, L, K, childProcessNumber, partialSize[19], N, splitSizes[19];
...
void writeResult(char *textName, int n, long long int *result){
  FILE *fp = fopen(textName, "w");
  for(int i = 1; i <= n; i++)
      fprintf(fp, "%d %lld\n", i, result[i]);
  fclose(fp);
...
void processPartial(char *message, long long int *result){
  char *ptr = strtok(message, "\n"); //parse at $
  int row, val;

  while(ptr) { //while token was found
    sscanf(ptr, "%d %d", &row, &val);
    result[row] += val;
    ptr = strtok(NULL, "\n");
  }
}
...
int main(int argc, char *argv[]){
  ...
  // K + 1 because my indexing will start from 1;
  int pipes[K + 1][2];
  for(int i = 1; i <= K; i++){
    if(pipe(pipes[i])){
      return printf("Couldn't create pipe = %d", i), -1;
    }
  }
  ...
  for(int i = 1; i <= K; i++){
    ...
    if(pid == 0){
      ...
      // printf("In the child process number %d and split size %d\n", i, m);
      ...
      for(int j = 1; j <= n; j++){
        if(visited[j] == 1){
          sprintf(currentLine, "%d %d\n", j, partial[j]);
          strcat(message, currentLine);
        }
      }

      // printf("message:\n%s", message);

      int r = write(pipes[i][1], message, sizeof(message));
      close(pipes[i][1]);
